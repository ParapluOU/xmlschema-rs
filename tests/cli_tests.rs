...
/// Test that unbounded elements followed by optional elements validate correctly.
///
/// Schema defines: sequence(title, author[1..∞], published[0..1], pages[0..1])
/// Document has: title, author, author, published, pages
///
/// This tests that the content model visitor correctly advances past unbounded
/// elements when their minOccurs is satisfied, allowing subsequent optional elements.
#[test]
fn test_validate_unbounded_followed_by_optional() {
    ...
    assert!(output.status.success(), "validation should succeed for valid document with unbounded+optional");
    assert!(stdout.contains("Document is valid"), "should report document valid");
}
